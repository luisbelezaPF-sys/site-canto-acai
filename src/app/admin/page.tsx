...
export default function AdminPage() {
  ...
  const baixarCupomPDF = async (pedido: Pedido) => {
    try {
      // Buscar URL do cupom no banco
      ...
      if (cupomData?.pdf_url) {
        // Se já existe, abrir
        window.open(cupomData.pdf_url, '_blank')
      } else {
        // Gerar novo cupom
        ...
        const cupomBlob = gerarCupomPDF({
          ...
          total: pedido.total,
          dataHora: new Date(pedido.data_hora).toLocaleString('pt-BR'),
        })

        // Baixar localmente
        const url = URL.createObjectURL(cupomBlob)
        const a = document.createElement('a')
        a.href = url
        a.download = `cupom-${pedido.id.slice(0, 8)}.pdf`
        a.click()
  ...
  const imprimirCupomPedido = async (pedido: Pedido) => {
    try {
      // Buscar URL do cupom no banco
      const { data: cupomData } = await supabase
        .from('cupons')
      ...
      if (cupomData?.pdf_url) {
        // Se já existe, imprimir
        imprimirCupom(cupomData.pdf_url)
      } else {
        // Gerar HTML para impressão direta
        const subtotal = pedido.total - pedido.taxa_entrega
        const conteudo = `
          <html>
            <head>
            ...
            <body>
              ...
              ${pedido.itens.map((item, index) => `
                <div class="item">
                  ...
                  ${item.acompanhamentos && item.acompanhamentos.length > 0 ? `<br>&nbsp;&nbsp;Acomp: ${item.acompanhamentos.join(', ')}` : ''}
              ...
              <hr>
              <p class="center">Obrigado por comprar no</p>
              <p class="center">Canto do Acai!</p>
            </body>
          </html>
        `
      ...
      <>
        ...
        <main className="min-h-screen pt-20 pb-8 px-4 relative z-10 flex items-center justify-center">
          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8 md:p-12 max-w-md w-full">
            <h1 className="text-3xl md:text-4xl font-bold text-[#FFC300] mb-6 text-center">
              Painel Admin
            </h1>
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full p-3 rounded-xl bg-white/10 text-white border border-white/20 focus:border-[#FFC300] focus:outline-none"
                  placeholder="admin"
                />
              </div>
              <div>
              ...
              <button
                onClick={fazerLogin}
                className="w-full bg-[#FFC300] hover:bg-yellow-500 text-[#4B0082] font-bold py-3 px-6 rounded-xl transition-all"
              >
                Entrar
              </button>
            </div>
    ...
    <>
      ...
      <main className="min-h-screen pt-20 pb-8 px-4 relative z-10">
        <div className="container mx-auto max-w-7xl">
          <h1 className="text-3xl md:text-5xl font-bold text-[#FFC300] mb-8 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-8">
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 flex items-center space-x-4">
              <DollarSign className="w-12 h-12 text-[#FFC300]" />
              <div>
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 flex items-center space-x-4">
              <Calendar className="w-12 h-12 text-[#FFC300]" />
              <div>
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 flex items-center space-x-4">
              <TrendingUp className="w-12 h-12 text-[#FFC300]" />
              <div>
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 mb-8">
            <h2 className="text-2xl font-bold text-[#FFC300] mb-4 flex items-center">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={produtosRanking}>
                ...
                <YAxis stroke="#fff" />
                <Tooltip
                  contentStyle={{ backgroundColor: '#4B0082', border: 'none', borderRadius: '8px' }}
                  labelStyle={{ color: '#FFC300' }}
                />
                <Bar dataKey="quantidade" fill="#FFC300" />
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6">
            <h2 className="text-2xl font-bold text-[#FFC300] mb-4">Todos os Pedidos</h2>
            {carregando ? (
                  ...
                  >
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <button
                          onClick={() => imprimirCupomPedido(pedido)}
                          className="bg-[#4B0082] hover:bg-purple-700 text-white px-4 py-2 rounded-lg flex items-center space-x-2 text-sm"
                        >
